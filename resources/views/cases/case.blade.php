                                                ...
                                                <table id="users-table"
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr id="row-{{ $caseStaff->cs_id }}">
                                                            ...
                                                            <td id="truncated-text" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 200px; cursor: pointer;" onclick="expandText(this)">{{ $caseStaff->perfusionist_category ?? 'N/A' }}
                                                            </td>
                                                            <td>{{ $caseStaff->perfusionistStatusDetail->st_name ?? 'N/A' }}
                                                            </td>
                                                            <td>{{ $caseStaff->secondPerfusionistDetail->st_name ?? 'N/A' }}
                                                            </td>
                                                            <td id="truncated-text" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 200px; cursor: pointer;" onclick="expandText(this)">{{ $caseStaff->second_perfusionist_category ?? 'N/A' }}
                                                            </td>
                                                            <td>
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        ...
                        $("#bypassForm")[0].reset(); // Reset form after successful submission
                    } else {
                        $("#responseMessage").html(
                ...
                error: function(xhr) {
                    ...
                    $.each(errors, function(key, value) {
                        errorMessages += `<div class="alert alert-danger">${value}</div>`;
                    });
